...
def create_trained_reasoning_policy(
    train_config: _config.TrainConfig,
    checkpoint_dir: pathlib.Path | str,
    *,
    repack_transforms: transforms.Group | None = None,
    ...
    """Create a stateful reasoning policy (think/act) from a trained checkpoint.

    This helper mirrors `create_trained_policy(...)` but returns `ReasoningPolicy`,
    intended for Pi0Fuse checkpoints that expose `prefill/reason/act`.
    """

    repack_transforms = repack_transforms or transforms.Group()
    checkpoint_dir = download.maybe_download(str(checkpoint_dir))

    # ReasoningPolicy uses JAX inference path (prefill/reason/act).
    weight_path = os.path.join(checkpoint_dir, "model.safetensors")
    if os.path.exists(weight_path):
        raise ValueError(
            "create_trained_reasoning_policy only supports JAX checkpoints with params/. "
    ...
    for method_name in ("prefill", "reason", "act"):
        if not hasattr(model, method_name):
            raise ValueError(
                ...
                "ReasoningPolicy requires a Pi0Fuse-style model."
            )
    ...
    return _policy.ReasoningPolicy(
        model,
        transforms=[
            *repack_transforms.inputs,
            transforms.InjectDefaultPrompt(default_prompt),
        ...
        force_initial_reasoning=force_initial_reasoning,
    )


def create_trained_skill_reasoning_policy(
    train_config: _config.TrainConfig,
    checkpoint_dir: pathlib.Path | str,
    *,
    repack_transforms: transforms.Group | None = None,
    ...
    initial_scene_plan: str = "",
    force_initial_reasoning: bool = True,
) -> _policy.SkillReasoningPolicy:
    """Create a stateful reasoning policy (think/act) from a trained checkpoint.

    This helper mirrors `create_trained_policy(...)` but returns `SkillReasoningPolicy`,
    intended for Pi0Fuse checkpoints that expose `prefill/reason/act`.
    """
    ...
    # SkillReasoningPolicy uses JAX inference path (prefill/reason/act).
    weight_path = os.path.join(checkpoint_dir, "model.safetensors")
    if os.path.exists(weight_path):
        raise ValueError(
            "create_trained_reasoning_policy only supports JAX checkpoints with params/. "
    ...
    for method_name in ("prefill", "reason", "act"):
        if not hasattr(model, method_name):
            raise ValueError(
                f"Model loaded from {checkpoint_dir} is missing `{method_name}`; "
                "SkillReasoningPolicy requires a Pi0Fuse-style model."
            )
    ...
    return _policy.SkillReasoningPolicy(
        model,
        transforms=[
            *repack_transforms.inputs,
            transforms.InjectDefaultPrompt(default_prompt),
...
def create_trained_atomic_policy(
     train_config: _config.TrainConfig,
    checkpoint_dir: pathlib.Path | str,
    *,
    repack_transforms: transforms.Group | None = None,
    ...
    norm_stats: dict[str, transforms.NormStats] | None = None,
    pytorch_device: str | None = None,
    ) -> _policy.Policy | _policy.AtomicReasoningPolicy:
    """Create a policy from a trained checkpoint.
    ...
    if norm_stats is None:
        ...
        norm_stats = _checkpoints.load_norm_stats(checkpoint_dir / "assets", data_config.asset_id)
    extra_policy_kwargs = {}
    extra_policy_kwargs['use_ref_img'] = False
    extra_policy_kwargs['initial_scene_plan'] = ''

    return _policy.AtomicReasoningPolicy(
        model,
        transforms=[
            *repack_transforms.inputs,
            transforms.InjectDefaultPrompt(default_prompt),
