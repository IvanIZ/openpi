"""Pi0TraceVLASingle: MoE-free ablation of Pi0TraceVLAMoe.

Three-stream Gemma trunk (identical wiring to ``Pi0TraceVLAMoe``, MoE dropped):

  - Stream 0: PaliGemma 2B VLM (dense FFN; full FT here).
  - Stream 1: action expert — a **single dense** FFN (default ``gemma_300m``:
              width=1024, mlp_dim=4096, depth=18). One expert of
              ``trace_vla_moe``'s 5-expert action MoE, with routing dropped.
              Warm-started from pi05_base's ``mlp_1``.
  - Stream 2: trace expert — a **single dense, lightweight** FFN (default
              ``gemma_trace_small``: width=512, mlp_dim=2048, depth=18). One
              expert of ``trace_vla_moe``'s shrunk trace MoE, with routing
              dropped. Randomly initialized (shape mismatch vs pi05_base).

This is the ablation that asks: *what is the benefit of the MoE structure on the
trace and action generators?* So vs ``Pi0TraceVLAMoe`` the ONLY architectural
change is that both non-VLM streams are now single dense FFNs (``TraceBlock``
auto-dispatches to ``lora.FeedForward`` when ``num_local_experts <= 1``), and
the per-token skill one-hot ``hard_combine_weights`` is no longer consumed by
any FFN — every trunk call passes a tiny inert placeholder.

Deliberately UNCHANGED from ``Pi0TraceVLAMoe`` (this is an ablation):
  - The per-skill **completion head** stays a skill-routed per-skill MLP
    (``num_completion_experts`` experts, indexed by the dataset ``atomic_token``).
    This is the only routing left in the model.
  - All conditioning (semantic-target Fourier+AdaRMS, EE row-0 inpainting clamp,
    optional appended target-anchor row), the overlay-image action input, the
    dataset, the transforms, the three losses (action, trace, completion), and
    every training trick (anchor-age aug, scene/overlay dropout, trace
    perturbation, image augmentation) are inherited verbatim.
"""
...
from openpi.models import pi0_trace_vla_single_config as _config
...
class Pi0TraceVLASingle(_model.BaseModel):
    """Trace-augmented VLA with single dense action + single dense trace heads."""

    def __init__(self, config: _config.Pi0TraceVLASingleConfig, rngs: nnx.Rngs):
        super().__init__(config.action_dim, config.action_horizon, config.max_token_len)
        self.config = config
        self.pi05 = config.pi05
        if not self.pi05:
            raise ValueError("Pi0TraceVLASingle assumes pi05=True (adaRMS pathway).")

        # Number of skill-specific experts in the (only) routed component: the
        # per-skill completion head. The trace/action streams carry no routing.
        self.num_completion_experts = int(config.num_completion_experts)
        ...
        # Both expert streams are now single dense FFNs; both come from gemmoe's
        # dense variants (MoE-free, num_local_experts == 1).
        action_expert_config = _gemma.get_config(config.action_expert_variant)
        trace_expert_config = _gemma.get_config(config.trace_expert_variant)

        # Both non-VLM streams must be dense for this ablation: TraceBlock dispatches to
        # ``lora.FeedForward`` (dense ``mlp_{i}``) iff ``num_local_experts <= 1``.
        if int(getattr(action_expert_config, "num_local_experts", 1)) > 1:
            raise ValueError(
                f"action_expert_variant must be a single dense FFN (num_local_experts=1) for the "
                f"single variant; got num_local_experts={action_expert_config.num_local_experts}."
            )
        if int(getattr(trace_expert_config, "num_local_experts", 1)) > 1:
            raise ValueError(
                ...
                f"single variant; got num_local_experts={trace_expert_config.num_local_experts}."
            )
        ...
        # TraceBlock auto-dispatches per-stream FFN by config.num_local_experts: with both
        # stream 1 and stream 2 at num_local_experts == 1, both get a dense ``lora.FeedForward``
        # (``mlp_1`` / ``mlp_2``); stream 0 stays dense as always.
        ...
        # adaRMS on streams 1 (action) and 2 (trace) — matches TraceVLA / TraceVLAMoe.
        ...
        # ---------- Completion head (per-skill MLP; KEPT routed — not part of the ablation) ----------
    ...
    # -----------------------------------------------------------------------
    # Completion head (per-skill MLP; routing KEPT — not part of the ablation)
    # -----------------------------------------------------------------------
    @at.typecheck
    def _completion_predict(
        ...
        skill_one_hot = jax.nn.one_hot(skill_id, self.num_completion_experts, dtype=out.dtype)
        logit = jnp.einsum("bk,bk->b", skill_one_hot, out)
        return jax.nn.sigmoid(logit)

    # -----------------------------------------------------------------------
    # Helper: inert placeholder combine weights for the trunk.
    # -----------------------------------------------------------------------
    def _dummy_combine_weights(self, batch_size: int) -> at.Float[at.Array, "b _t _k"]:
        """Tiny inert ``hard_combine_weights`` placeholder.

        Neither non-VLM stream is an MoE in this ablation, so ``TraceBlock`` always
        takes the dense FFN branch and never reads ``hard_combine_weights``. The
        trunk's ``__call__`` still requires a well-shaped array, so we pass a
        ``(B, 1, 1)`` placeholder on every call (matches the ``K=1`` placeholder
        ``TraceModule.init`` builds when no stream has experts).
        """
        return jnp.zeros((batch_size, 1, 1), dtype=jnp.dtype(self.config.dtype))
    ...
    def _forward_planning(
        ...
        """Trace planning forward. Stream 2 (single dense trace FFN) is active.
        ...
        # No MoE on the trace stream — pass an inert placeholder for hard_combine_weights.
    ...
    def _forward_execution(
        ...
        """Action execution forward. Stream 1 (single dense action FFN) is active."""
        noise_rng, time_rng = jax.random.split(rng, 2)
        ...
        # No MoE on the action stream — pass an inert placeholder for hard_combine_weights.
        # The completion head still routes by skill id (the only routing left).
        if obs.atomic_token is None:
            raise ValueError("atomic_token is required for the per-skill completion head.")
    ...
    def compute_loss(
        ...
        *,
        ...
        # cameras. It is None for square sources (LIBERO), in which case preprocessing is
        # unchanged. Only the train=True path uses it; the inference (sample_*) paths do
        # not augment keypoints, so they need not pass it. (Identical to Pi0TraceVLAMoe.)
        ...
        # Trace planning (single dense trace stream).
        ...
        # Action execution (single dense action stream) + completion head.
    ...
    def sample_actions(
        ...
        *,
        ...
        """Sample actions from the (single dense) action head (execution mode).
        ...
        # No MoE anywhere — a single inert placeholder works for prefill and the loop.
        dummy_weights = self._dummy_combine_weights(batch_size)
